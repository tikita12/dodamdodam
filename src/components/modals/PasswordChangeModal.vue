...
async function handleSubmit() {
  ...
  if (!newPassword.value.trim() || newPassword.value.length < 4) {
    errorMessage.value = '새 비밀번호는 4자리 이상이어야 합니다.'
    return
  }
...
<template>
  ...
  >
    <div class="w-full max-w-sm bg-white rounded-3xl p-5 shadow-2xl border border-slate-100 flex flex-col space-y-4">
      <!-- Modal Header -->
      <div class="flex items-center justify-between pb-3 border-b border-slate-100">
        <div class="flex items-center gap-2">
          ...
          <div>
            <h3 class="text-sm font-bold text-slate-900 leading-tight">비밀번호 변경</h3>
            <p class="text-[11px] text-slate-400">{{ sessionStore.volunteerName }}님의 새 비밀번호 설정</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <form @submit.prevent="handleSubmit" class="space-y-3">
        <div>
          ...
          <input
            ...
            class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white"
          />
        </div>
        ...
        <div class="flex gap-2 pt-2 border-t border-slate-100">
          ...
          <button
            type="submit"
            :disabled="isLoading"
            class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-bold rounded-xl text-xs shadow-sm transition flex items-center justify-center gap-1 cursor-pointer"
          >
